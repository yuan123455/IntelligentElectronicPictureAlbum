<template>
	<view>
		...
		<view class="main" v-for="(item,i) in list" :key="i">
			...
			<button @click="bian(item.id)">修改审核</button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getList(){
				my.getData("mobile/shopadmin/FxList","sign=0",getApp().globalData.key).then(res=>{
					console.log(res);
					this.list=res.data.result.list.list;
				})
			},
			downLoad(){
				my.load("mobile/shopadmin/FxList","sign=0",this.pag,getApp().globalData.key).then(res=>{
